...
exports.generateFallbackGuide = generateFallbackGuide;
exports.generateFallbackExercise = generateFallbackExercise;
exports.createApiResponse = createApiResponse;
exports.validateQuizData = validateQuizData;
/**
 * Generate fallback guide when Gemini is not available
 */
function generateFallbackGuide(quizLevels, subjective) {
    ...
    if ((subjective === null || subjective === void 0 ? void 0 : subjective.breakPreference) && subjective.breakPreference.toLowerCase().includes('stretch')) {
        exercises.push("Combine eye exercises with gentle neck and shoulder stretching");
    }
    if ((subjective === null || subjective === void 0 ? void 0 : subjective.favoriteSnack) && subjective.favoriteSnack.toLowerCase().includes('coffee')) {
        exercises.push("Take coffee breaks while practicing distance focusing");
    }
    if ((subjective === null || subjective === void 0 ? void 0 : subjective.favoritePlace) && subjective.favoritePlace.toLowerCase().includes('window')) {
 ...
 */
function generateFallbackExercise(userPreferences, breakCount) {
    const exercises = [
        {
    ...
    if (userPreferences && userPreferences.length > 0) {
        ...
        if (preferences.includes('tea') || preferences.includes('coffee') || preferences.includes('drink')) {
            selectedExercise = Object.assign(Object.assign({}, selectedExercise), { message: `Prepare your favorite beverage while looking outside. ${selectedExercise.message}` });
        }
        if (preferences.includes('music')) {
            selectedExercise = Object.assign(Object.assign({}, selectedExercise), { message: `With your favorite music, ${selectedExercise.message}` });
        }
    }
 ...
 */
function createApiResponse(success = true, message, data, source, error) {
    return {
        success,
        message,
        data,
 ...
 * Validate quiz data
 */
function validateQuizData(userId, quiz) {
    if (!userId) {
