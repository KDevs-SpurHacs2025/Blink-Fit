import { FilesetResolver, FaceLandmarker, DrawingUtils } from './task-vision/vision_bundle.mjs';
    ...
    if (results.faceLandmarks && results.faceLandmarks.length > 0) {
        ...
        if (currentEyeClosed && !prevEyeClosed) {
            ...
            if (window.chrome && chrome.runtime) {
                chrome.runtime.sendMessage({ type: 'BLINK_DETECTED', blinkCount: blinkCount });
            }
        }
        prevEyeClosed = currentEyeClosed;

        const drawingUtils = new DrawingUtils(canvasCtx);
        drawingUtils.drawConnectors(landmarks, FaceLandmarker.FACE_OVAL, { color: '#E0E0E0' });
        drawingUtils.drawConnectors(landmarks, FaceLandmarker.LEFT_EYE, { color: '#FF3030' });
        drawingUtils.drawConnectors(landmarks, FaceLandmarker.RIGHT_EYE, { color: '#30FF30' });
    }
}
